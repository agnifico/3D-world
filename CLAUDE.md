...
- No localStorage/sessionStorage — state lives in memory. Sanctioned exception: `world/zones/gallery/marks.js`'s `gallery-marks` key — dev-inspection tooling for judging the asset catalogue, never shipped as game state, and the judging pass explicitly needs to survive a reload. Any other localStorage use still needs to clear this bar (dev tool, not game state) or get asked about first.
...
8. Before running a shell command whose effect isn't obvious from the command text itself, give a one-line plain-English rationale first.
