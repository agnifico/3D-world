...
export const CHARACTERS = {
  mia: {
    ...
    clips: {
      ...
      hopOut: 'Jumping Out Of A Plane.fbx', tread: 'Treading Water.fbx',
...
export const DEFAULT_CHARACTER = 'vic';
...
// VRoid/MToon GLB exports are inconsistent: some ship lit MeshStandardMaterial
// (Mia), others unlit MeshBasicMaterial (Vic) that ignore scene lights entirely
// — so the same character would look identical day and night while others
// darken correctly. Convert any unlit material to a lit Standard one, carrying
// the texture/color/alpha across, so EVERY character obeys the world lighting
// (incl. the night palette + lantern) the same way. Standard mats pass through.
function toLitMaterial(m) {
  ...
  const lit = new THREE.MeshStandardMaterial({
    ...
    alphaMap: m.alphaMap || null, alphaTest: m.alphaTest,
    side: m.side, vertexColors: m.vertexColors, name: m.name,
    roughness: 0.9, metalness: 0.0,   // toon skin/cloth: matte, non-metallic — catches soft moonlight without specular blowout
  });
  lit.toneMapped = m.toneMapped;
  return lit;
}
    ...
    model.traverse(o => {
      ...
      o.material = Array.isArray(o.material) ? o.material.map(toLitMaterial) : toLitMaterial(o.material);
    });
