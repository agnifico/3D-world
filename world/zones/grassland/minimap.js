// Grassland zone — minimap: two baked aerials (day/night, 1024px — high
...
const ZOOM_LEVELS = [70, 160]; // world-unit radius shown; second entry (>= WORLD_HALF) clamps to the full map
let zoomIdx = 0;
let _mm = null;

export function initMinimap(ctx, animated, PALETTES, boats, getChar, getHeading, isOverlayOpen) {
  const _M = 172; // on-screen display size
  const _baseDay = bakeAerial(PALETTES.day);
  const _baseNight = bakeAerial(PALETTES.night);
  _mm = document.createElement('canvas'); _mm.width = _mm.height = _M;
  ...
  addEventListener('keydown', e => { if (e.isTrusted && e.code === 'KeyM') zoomIdx = (zoomIdx + 1) % ZOOM_LEVELS.length; }, { signal: ctx.abortController.signal });

  let _mmT = 1; // primed so the first frame draws overlays without waiting on the throttle
  let _wasNight = null; // frame-styling latch: only touch the DOM when the mode flips
  animated.push(dt => {
    _mmT += dt; if (_mmT < 0.09) return; _mmT = 0;
    if (isOverlayOpen() || window.__aerial) { _mm.style.display = 'none'; return; }
    _mm.style.display = '';
    ...
    const night = ctx.lighting.getT() >= 0.5;             // hard swap at t=0.5, per the brief
    c.drawImage(night ? _baseNight : _baseDay, crop.sx, crop.sy, crop.sSize, crop.sSize, 0, 0, _M, _M);
    if (night) { c.fillStyle = 'rgba(12,14,38,0.34)'; c.fillRect(0, 0, _M, _M); } // dim the aerial so it doesn't glow like a torch in the corner
    if (night !== _wasNight) {                          // cool the frame at night — styling only
      _wasNight = night;
      _mm.style.borderColor = night ? 'rgba(126,146,196,0.5)' : 'rgba(107,79,53,.6)';
      _mm.style.boxShadow = night ? '0 8px 20px rgba(4,6,22,0.6)' : '0 6px 18px rgba(0,0,0,.4)';
    }
...
// DOM cleanup — the minimap canvas isn't part of the THREE scene graph, so
// the zone's generic disposeGroup() traversal never sees it; dispose() must
// remove it explicitly. The keydown listener is covered by abortController.
export function disposeMinimap() {
  if (_mm) { _mm.remove(); _mm = null; }
}
